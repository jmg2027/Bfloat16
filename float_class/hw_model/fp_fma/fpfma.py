...
class FloatSummation:
    ...
    #        sign_v, exp_v, mant_v = zip(*decompsed_vector)
            ...
            exp_max_tree: List[List[bit]] = [[bit(fp32_config.exponent_bits, '0'*(fp32_config.exponent_bits))] * (2**(tree_level-i)) for i in range(tree_level+1)]
            # Initialize
            ...
            shamt: List[int] = []
            for i in range(len(exp_v_signed)):
                if int(o_max_exp - exp_v_signed[i]) >= 32:
                    shamt.append(32)
                else:
                    shamt.append(int(o_max_exp - exp_v_signed[i]))
            # Acc shift
            if int(o_max_exp - exp_acc_signed) >= 32:
                shamt.append(32)
            else:
